<template>
  ...
  <div class="mx-auto bg-gray-50">
    <div class="mx-auto max-w-3xl pt-12 pb-20">
      ...
      <form class="space-y-6" @submit.prevent="submitTextForTitle">
        <div class="bg-white px-4 py-5 shadow sm:rounded-lg sm:p-6">
          ...
          <div class="flex justify-start mt-8">
            <button type="submit" :disabled="titlesLoading ? true : false" class="flex items-center rounded-md border border-transparent py-2 px-4 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2" :class="titlesLoading ? 'bg-gray-400 cursor-default hover:bg-gray-400' : 'bg-sky-600 focus:ring-sky-500 hover:bg-sky-700'">
              {{ titlesLoading ? 'Generating Titles...' : 'Next' }}
      ...
      <div v-show="bodyLoading || bodyOutput" id="blog-post" class="relative bg-white px-4 py-5 mt-8 shadow sm:rounded-lg sm:p-6">
        ...
        <form v-show="bodyLoaded && !emailSubmitted" id="emailForm" method="POST" action="https://script.google.com/macros/s/AKfycbx8-so2HIpbI7riAASaZ663Fv8LSSkqOrfHoBrFd-WLkBgOflcTGX3v6vlVyyOAEYQY/exec" target="hiddenFrame" class="space-y-6 absolute top-60 left-0 right-0 m-auto w-3/4">
          <div class="bg-white px-4 py-5 shadow sm:rounded-lg sm:p-6 justify-center">
            ...
            <div class="flex justify-start mt-8">
              <button type="submit" :disabled="!emailSubmitting ? false : true" :class="emailSubmitting ? 'bg-gray-400 cursor-default hover:bg-gray-400' : 'bg-sky-600 hover:bg-sky-700 focus:ring-sky-500'" class="flex items-center rounded-md border border-transparent py-2 px-4 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2">
                {{ !emailSubmitting ? 'Submit' : 'Loading...' }}
                <svg v-if="emailSubmitting" aria-hidden="true" role="status" class="inline w-4 h-4 ml-2 text-gray-200 animate-spin dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                  <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="#ffffff" />
                </svg>
              </button>
...
export default {
  ...
  methods: {
    ...
    submitTitleForBody(e) {
      ...
      // Scroll to blog post section
      this.$nextTick(function () {
        document.getElementById('blog-post').scrollIntoView({
          behavior: 'smooth'
        })
      })
